...
@dp.table(
name="finance.gold.fraud_card_alert"
...
def fraud_card_alert() -> DataFrame:
    transactions=spark.readStream.table("finance.silver.transactions")
    fraud_watchlist=spark.readStream.table("finance.silver.fraud_watchlist")

    customers=spark.read.table("finance.silver.customers")
